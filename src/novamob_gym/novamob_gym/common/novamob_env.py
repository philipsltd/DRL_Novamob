...
class NovamobGym(gym.Env):
    ...
    def lidar_callback(self, msg):
        with self.lidar_lock:
            ...
            lidar_readings = [min(x, 10.0) if x == float('inf') else x for x in lidar_readings]
    ...
    def step(self, action):
        ...
        # Calculate the goal distance, reward and check if the episode is done
        done = self.is_done()

        print(f"Old Goal Distance: {self.goal_distance}")
        ...
        print(f"New Goal Distance: {self.goal_distance}")
